...
export const runDistanceTests = () => {
  ...
  console.log(`✅ Same location distance: ${sameLocation.toFixed(3)}km (expected: 0)`);
  ...
  console.log(`✅ North-South distance: ${northSouth.toFixed(3)}km (expected: ~2km)`);
  ...
  console.log(`✅ East-West distance: ${eastWest.toFixed(3)}km (expected: ~2km)`);
...
export const runDeliveryFeeTests = () => {
  ...
  testCases.forEach(({ distance, expected }) => {
    ...
    console.log(`${status} Distance: ${distance}km → Fee: $${fee} (expected: $${expected})`);
  });
...
export const runTimeEstimationTests = () => {
  ...
  testCases.forEach(({ distance, expectedMin, expectedMax }) => {
    ...
    console.log(`${status} Distance: ${distance}km → Time: ${time}min (expected: ${expectedMin}-${expectedMax}min)`);
  });
